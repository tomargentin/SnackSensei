...
def page11(next_page, prev_page):
    ...
    if "selected_stores" in st.session_state:
        ...
        # Ensure only available stores are selected
        stores_to_query = [store for store in selected_stores if store in available_stores]
        ...
        if response.status_code == 200:
            ...
            for item, details in store_data.items():
                if details['price'] != "NA":
                    item_price = float(details['price']) * ingredients[item]
